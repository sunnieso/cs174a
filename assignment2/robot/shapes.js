 ...
 // compound shapes by populating them with a pre-transformed triangle
{
	...
	if( !arguments.length) return; // Pass no arguments if you just want to make an empty dummy object that inherits everything, for populating other shapes (same as static)
...
inherit(triangle, shape);
	...
	{
		var offset = recipient.vertices.length; var index_offset = recipient.indices.length; // If recipient is not the triangle, get its previous size
		...
		for( var i = offset; i < recipient.vertices.length; i++ ) // Apply points_transform to all points added during this call
		recipient.vertices[i] = vec3( mult_vec( points_transform, vec4( recipient.vertices[ i ], 1 ) ) );
	}
...
// // /////
function tetrahedron_share() 
{
	shape.call(this); // Inherit class shape’s array members by calling parent constructor
	this.populate(); // Otherwise, a new triangle immediately populates its own arrays with triangle points,
	this.init_buffers(); // Then sends its arrays to the graphics card into new buffers
}
inherit(tetrahedron_share, shape);
	tetrahedron_share.prototype.populate = function( ) // The meat of the triangle starts here:
	{
		var a = 1/Math.sqrt(3);


		this.vertices.push( vec3(0,0,0), vec3(2,0,0), vec3(1,0,a), vec3(1,1,a/2));
		...
		this.indices.push(0,1,2,0,1,3,0,2,3,1,2,3); // vertices shared

		// this.vertices.push( vec3(0,0,0), vec3(0,a,0), vec3(1,0,0), vec3(0,0,1));
		// this.normals.push(vec3(a,a,a), vec3(0,1,0), vec3(1,0,0), vec3(0,0,1));
		// this.texture_coords.push(vec2(0,0), vec2(0,1),vec2(1,0),vec2(1,1));
		// this.indices.push(0,1,2,0,1,3,0,2,3,1,2,3); // vertices shared
	};
...
// }
// inherit(windmill, shape);
// 	shape1.prototype.populate = function( num_blades )
// 	{
// 	 this.vertices.push( vec3( 0, 0, 0 ) );
// 	 this.vertices.push( vec3( 1, 0, 0 ) );
// 	 this.vertices.push( vec3( 0, 0, 0 ) );
// 	 this.vertices.push( vec3( 0, 0, 0 ) );
// 	 this.normals. push( vec3( 0, -1, 0 ) ); // (Not planning on flat shading)
// 	 this.texture_coords.push( vec2( 0, 0 ) );
