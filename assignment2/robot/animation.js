...
var texture_filenames_to_load = [	"transOrange.png" , 
									"text.png", 
									"brick.jpg", 
									"crackedSoil.jpg", 
									"grass.jpg",
...
var purplePlastic = new Material( vec4( .9,1,.9,1 ), .2, .5, .8, 40 ), // Omit the final (string) parameter if you want no texture
	greyPlastic = new Material( vec4( .5,.5,.5,1 ), 0.5, .1, .5, 20, "robot.jpg" ),
	rusty	 	= new Material( vec4( .5,.5,.5,1 ), 0.5, .1, .5, 20, "rusty.jpg"),
	bluePlastic = new Material( vec4( .0,.0,.9,1 ), .2, .4, .3, 30 ),
	// soil 		= new Material( vec4( .7,.9,.5,1 ), .5, .1, .5, 10, "crackedSoil.jpg" ),
	red 		= new Material( vec4( .9,0,0,1 ), 1, .5, .8, 40 ), 
	yellow 		= new Material( vec4( 1,1,0,1 ), 1, .5, .8, 40 ),
	yellow1		= new Material( vec4( 1,1,0,1 ), 0.5, .5, .8, 40 ),
	purple 		= new Material( vec4( .5,0,.5,1 ), 1, .5, .8, 40 ), 
	lightBlue 	= new Material( vec4( .3,.3,.5,1 ), 1, .5, .8, 40 ), 
	 
	water 		= new Material( vec4( .0,.0,.3,0.9), 1, .5,  0.1, 10 ),
	greenGrass 	= new Material( vec4( 0,0.8,0.5,1), 1, .5,  0.1, 30, "grass.jpg" ),
	wood 		= new Material( vec4( 0,0,0,1 ),  1, 0.0001,   1, 10000, "wood.jpg"),
	soil 		= new Material( vec4( 0,0,0,1 ),  1, 0.1,   1, 10000, "sand.jpg"),
	pyramid		= new Material( vec4( 0,0,0,1 ),  1, 0.001,   1, 10000, "pyramid.jpg"),
	sea 		= new Material( vec4( 0,0,0,1 ),  1, 0.0001,   1, 10000, "sea.jpg"),
	desert 		= new Material( vec4( 0,0,0,1 ),  1, 0.001,   1, 10000, "desert.jpg"),
	...
	sign	 	= new Material( vec4( 0,0,0,1 ),  1, 0.0001,   1, 10000, "sign.jpg"),
...
function Animation()
{
	( function init (self) 
	{
		...
		self.tetrahedron_share = new tetrahedron_share();
...
function drawRobot(parent, model_transform){
	...
	// if on the desert, get the robot walk higher
	body_transform = mult(body_transform, translation(0,2,0));
...
// RAIN
var pouringRainId = 0;
var rainRangeStart = [-200, 80,-10] ;
var rainRangeDis = [200,80,20];
var rainAmount = 200;
var Rain = [];
...
var firePos = [];
var fireColor = [];
function initFireWorksPos(){
	for ( var i = 0; i != 5; i++){
		firePos.push(vec3(25-50*Math.random(),40-10*Math.random(),-80));
	}
	fireColor.push(yellow);
	fireColor.push(red);
	fireColor.push(red);
	fireColor.push(lightBlue);
	fireColor.push(purple);
}
...
Animation.prototype.display = function(time)
	{
		...
		// fire works (custom shape)
		if(camera_mode == CAMERA_WHOLE_VIEW){
			for ( var i = 0; i != 4; i++){
				model_transform = mtStack.pop();
				mtStack.push(model_transform);
				model_transform = mult(model_transform, translation(firePos[i][0],firePos[i][1],firePos[i][2]));
				model_transform = mult(model_transform, rotation(90,1,0,0));
				var sc = (Math.sin(this.graphicsState.animation_time/500+ i*10)+1)*5;
				model_transform = mult(model_transform, scale(sc,sc,sc));
				this.m_windmill.draw( this.graphicsState, model_transform, fireColor[i] );
			}
		}
		...
		if(camera_mode == CAMERA_WHOLE_VIEW){	// final scene	
			if( fallTime <= EndTime){			// calculate the falling angle and respected time
				model_transform = mult(model_transform, rotation(fallTime, 0,0,-1)); 
				fallTime+=1;
				model_transform = mult(model_transform, translation(0,+50,-75));
				model_transform = mult(model_transform, scale(5,100,150));
				...
				mtStack.push(model_transform);
				model_transform = mult(model_transform, rotation(fallTime, 0,0,-1)); 
				model_transform = mult(model_transform, translation(100,0,10));
				model_transform = mult(model_transform, scale(10,10,10));
				this.tetrahedron_share.draw( this.graphicsState, model_transform, pyramid );

				model_transform = mtStack.pop();
				mtStack.push(model_transform);
				model_transform = mult(model_transform, rotation(fallTime, 0,0,-1)); 
				model_transform = mult(model_transform, translation(10,0,-50));
				model_transform = mult(model_transform, scale(40,20,20));
				this.m_tetrahedron.draw( this.graphicsState, model_transform, yellow1 );
			} else if ( fallTime <= EndFlashTime){		// calculate wall flashing period
		...
		if(camera_mode == CAMERA_WHOLE_VIEW){			
			if( fallTime <= EndTime){
				...
				model_transform = mtStack.pop();
				// sign
				model_transform = mult(model_transform, translation(50,50,-80));
				model_transform = mult(model_transform, scale(50,25,5));
				this.m_cube.draw( this.graphicsState, model_transform, sign );			
			}
		} else {
			model_transform = mult(model_transform, translation(200,10,-80));
			model_transform = mult(model_transform, scale(400,300,1));
			...
			// side background (for pouring view)
			model_transform = mtStack.pop();
			mtStack.push(model_transform);
			model_transform = mult(model_transform, translation(-500,40,2500));
			model_transform = mult(model_transform, rotation(90,0,1,0));
			model_transform = mult(model_transform, scale(5000,80,1));
		...
		if(camera_mode == CAMERA_WHOLE_VIEW){	// final scene	
			if( fallTime > EndTime){			// calculate the falling angle and respected time
				if ( fallTime <= EndFlashTime){		// calculate wall flashing period	
					if(flashClk){
						model_transform = mult(model_transform, translation(250,1,0));
						model_transform = mult(model_transform, scale(500,1,250));
						this.m_cube.draw( this.graphicsState, model_transform, soil );
					}
				}
			}else{
				model_transform = mult(model_transform, translation(250,1,1));
				model_transform = mult(model_transform, scale(500,1,250));
				this.m_cube.draw( this.graphicsState, model_transform, soil );
			}
		} else {
		...
		model_transform = mtStack.pop();
		mtStack.push(model_transform);
		model_transform = mult(model_transform, translation(Math.sin(this.graphicsState.animation_time/500),0,0));
		model_transform = mult(model_transform, scale(1000,1,250));
		this.m_cube.draw( this.graphicsState, model_transform, sea );

		// Walkway
		model_transform = mtStack.pop();
		mtStack.push(model_transform);
		model_transform = mult(model_transform, translation(0,1,0));
		model_transform = mult(model_transform, scale(10,2,200));
		this.m_cube.draw( this.graphicsState, model_transform, wood );
...
// clock divider for flashing wall.
function update_clk(){
	if(cntFlashClk == 8){
		cntFlashClk = 0;
		flashClk = ~flashClk;
...
Animation.prototype.update_strings = function( debug_screen_strings )		// Strings this particular class contributes to the UI
{
	...
	debug_screen_strings.string_map["frame"] = "Frame rate: " + 1/(this.graphicsState.animation_time/1000);
